...
class DecoupleLayer(nn.Module):
    def __init__(self, in_c=2048, out_c=128):
...
class output_block(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # x1 is 1/4, x2 is 1/8, x3 is 1/16
        x2 = self.up_2x2(x2) # -> 1/4
        x3 = self.up_4x4(x3) # -> 1/4
...
class Baseline6Network(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        self.layer4 = backbone.layer4  # [batch_size, 2048, h/32, w/32]

        """ FEM """
        self.dconv1 = dilated_conv(256, 128)  # from layer1
        self.dconv2 = dilated_conv(512, 128)  # from layer2
        self.dconv3 = dilated_conv(1024, 128) # from layer3
        self.dconv4 = dilated_conv(2048, 128) # from layer4

        """ Decouple Layer """
        self.decouple_layer = DecoupleLayer(2048, 128)

        """ Adjust the shape of decouple output """
        self.preprocess_fg4 = CDFAPreprocess(128, 128, 1)  # 1/32 -> 1/32
        self.preprocess_bg4 = CDFAPreprocess(128, 128, 1)  

        self.preprocess_fg3 = CDFAPreprocess(128, 128, 2)  # 1/32 -> 1/16
        self.preprocess_bg3 = CDFAPreprocess(128, 128, 2)  

        self.preprocess_fg2 = CDFAPreprocess(128, 128, 4)  # 1/32 -> 1/8
        self.preprocess_bg2 = CDFAPreprocess(128, 128, 4)  

        self.preprocess_fg1 = CDFAPreprocess(128, 128, 8)  # 1/32 -> 1/4
    ...
    def forward(self, image):
        """ Backbone: ResNet50 """
        x0 = image
        x_pre = self.layer0(x0)  # 1/2
        x1 = self.layer1(x_pre)  # 1/4
        ...
        """ Decoder """
        f_small = self.decoder_small(f2, f1) # 1/8 -> 1/4
        f_middle = self.decoder_middle(f3, f2) # 1/16 -> 1/8
        f_large = self.decoder_large(f4, f3) # 1/32 -> 1/16
