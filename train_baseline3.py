...
from models.baseline3_net import Baseline3Network
...
if __name__ == "__main__":
    ...
    folder_name = f"Baseline3_CR_{dataset_name}_{val_name}_lr{lr_decoder}_{current_time}"
    ...
    # 实例化 Baseline 3 (包含 SID + CDFA)
    model = Baseline3Network()
    
    # ---------------------------------------------------------
    # 第二阶段权重加载逻辑 (Stage 2 - CR Pretrained weights)
    # ---------------------------------------------------------
    if os.path.exists(stage1_pretrained_path):
        ...
        if len(filtered_dict) == 0:
            raise ValueError(f"CR Weight Transfer Failed: No keys matching {backbone_prefixes} were found in {stage1_pretrained_path}.")
    ...
    # 完全剥离辅助损失，仅使用基本的 BCE+Dice 计算主掩码 loss
    loss_fn = DiceBCELoss()
