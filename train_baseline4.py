...
from models.baseline4_net import Baseline4Network
...
def train_baseline(model, loader, optimizer, loss_fn, device):
    ...
    for i, (x, (y1, y2)) in enumerate(tqdm(loader, desc="Training", total=len(loader))):
        ...
        y2 = y2.to(device, dtype=torch.float32)  # [新增] 读取背景掩码
        ...
        # [修正] 直接使用网络输出的张量进行互补损失计算，移除错误的 Softmax 挤压
        prob_fg, prob_bg, prob_uc = fg_pred, bg_pred, uc_pred
        ...
        # [修改] 仅记录主掩码 loss_mask.item()，确保与无损失版的 Loss 日志可以直接对比：
...
def evaluate_baseline(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for i, (x, (y1, y2)) in enumerate(tqdm(loader, desc="Evaluation", total=len(loader))):
            x = x.to(device, dtype=torch.float32)
            y1 = y1.to(device, dtype=torch.float32)
            y2 = y2.to(device, dtype=torch.float32) # [新增]
...
if __name__ == "__main__":
    ...
    lr_encoder = 1e-4  # Encoder smaller LR
    early_stopping_patience = 40

    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    # 标注 folder_name 为带有 SID_Loss 的实验
    folder_name = f"Baseline4_SID_Loss_CR_{dataset_name}_{val_name}_lr{lr_decoder}_{current_time}"
    ...
    # 实例化带有辅助预测输出的 Baseline4Network
    model = Baseline4Network()
    ...
    if os.path.exists(stage1_pretrained_path):
        ...
        # 仅保留 ResNet-50 的权重。在 Net0 (ConDSegStage1) 中，前缀对应为 layer0~layer4
    ...
    print_and_save(train_log_path, "Optimizer: Adam with Parameter Groups (Encoder: 1e-4, Decoder: 1e-4)\nScheduler: CosineAnnealingLR\nLoss: BCE Dice Loss + SID Losses\n")
